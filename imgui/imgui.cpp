...
static ImVec2 FindBestPopupWindowPos(const ImVec2& base_pos, const ImVec2& size, ImGuiWindowFlags flags, int* last_dir, const ImRect& r_inner)
{
    ...
    {
        const int mDir = (n == -1) ? *last_dir : n;
        ImRect rect(mDir == 0 ? r_inner.Max.x : r_outer.Min.x, mDir == 1 ? r_inner.Max.y : r_outer.Min.y, mDir == 3 ? r_inner.Min.x : r_outer.Max.x, mDir == 2 ? r_inner.Min.y : r_outer.Max.y);
        if (rect.GetWidth() < size.x || rect.GetHeight() < size.y)
            continue;
        *last_dir = mDir;
        return ImVec2(mDir == 0 ? r_inner.Max.x : mDir == 3 ? r_inner.Min.x - size.x : base_pos_clamped.x, mDir == 1 ? r_inner.Max.y : mDir == 2 ? r_inner.Min.y - size.y : base_pos_clamped.y);
    }
