...
public:
	...
	void Render(const Camera &cam, int side, SphereParamsStorage *parent);
	void Update(const Camera &camera, float Rs, float eps, int max_divide, SphereParamsStorage *parent);
	void GenerateSubTexture(std::shared_ptr<Material> &t, SphereParamsStorage *parent, float devider = 1, bool = false, const glm::vec4 & part = {0,0,1,1});
	void GenTex(SphereParamsStorage *parent, std::shared_ptr<Texture> height_map, std::shared_ptr<Texture> grad_map, float _res, const glm::vec4 & part = {0,0,1,1});
